    ...
    if (!world) {
        return (
            <div className="text-center p-12 animate-fade-in">
                <h2 className="text-2xl text-foreground">Realm not found in this chronicle.</h2>
                <Link to="/worlds" className="text-primary mt-4 inline-block">Return to the Atlas</Link>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-3xl mx-auto p-4 sm:p-8 animate-fade-in">
            ...
            <header className="mb-8 pb-4 border-b border-border">
                <h1 className="text-5xl font-bold font-serif text-foreground">{world.name}</h1>
                <p className="text-lg text-primary font-semibold mt-1">{world.theme}</p>
                <p className="text-sm text-foreground/70 mt-2">{world.setting}</p>
            </header>
                    ...
                    <Card>
                        {linkedChars.length > 0 && (
                            <div className="space-y-1">
                                <h3 className="text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2">🧝 Known Inhabitants</h3>
                                {linkedChars.map(char =>
                                    <Link key={char.id} to={`/characters/${char.id}`} className="text-sm text-primary hover:underline block">- {char.name}</Link>
                                )}
                            </div>
                        )}
                        {linkedWritings.length > 0 && (
                            <div className={`space-y-1 ${linkedChars.length > 0 ? 'mt-4' : ''}`}>
                                <h3 className="text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2">✍️ Associated Lore</h3>
                        ...
                        {linkedEvents.length > 0 && (
                            <div className={`space-y-1 ${(linkedChars.length > 0 || linkedWritings.length > 0) ? 'mt-4' : ''}`}>
                                <h3 className="text-xs font-bold text-foreground/70 uppercase tracking-wider mb-2">⏳ Historical Events</h3>
                                {linkedEvents.map(event =>
                                    <Link key={event.id} to={`/timeline`} className="text-sm text-primary hover:underline block">- {event.title}</Link>
                                )}
                            </div>
                        )}
                    </Card>
            ...
            <div className="mt-12 text-center">
                <Link to="/worlds" className="text-primary hover:text-foreground transition-colors">
